...
const FacultyAttendance = () => {
  ...
  const filtered = useMemo(() => {
    return faculty.filter((emp) => {
      ...
      const matchLoc =
        locFilter === "All" || getLocation(emp.empId) === locFilter;
      const matchActive =
  ...
  return (
    <div className="flex h-[calc(100vh-130px)] gap-0 overflow-hidden rounded-xl border border-gray-200 bg-white">
      ...
      ══════════════════════════════ */}
      <div
        className={`flex flex-col border-r border-blue-200 transition-all duration-300 flex-shrink-0 ${
          collapsed ? "w-14" : "w-72"
        }`}
      >
        {/* Sidebar Header */}
        <div className="flex items-center justify-between px-3 py-3 border-b border-blue-200 flex-shrink-0">
          {!collapsed && (
          ...
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-blue-200 text-blue-400 hover:text-blue-600 transition-colors ml-auto flex-shrink-0"
            ...
            <div className="space-y-2 p-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="flex items-center gap-2 animate-pulse">
                  <div className="w-8 h-8 rounded-full bg-blue-800 flex-shrink-0" />
                  {!collapsed && (
                    <div className="flex-1 space-y-1.5">
                      <div className="h-3 bg-blue-800rounded w-3/4" />
            ...
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-100">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${LOC_STYLE[getLocation(selectedEmp.empId)]}`}>
                    {getLocation(selectedEmp.empId)}
                  </span>
